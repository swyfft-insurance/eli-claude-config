...
def main():
    if len(sys.argv) < 2:
        print("Usage: read-ticket.py <ISSUE-ID> [TICKET-FOLDER]", file=sys.stderr)
        sys.exit(1)

    issue_id = sys.argv[1].upper()
    # Ticket-folder name to dump under ~/.claude/tickets/<folder>/artifacts/. Optional: defaults to
    # the issue ID (correct for a first read, before a plan folder exists). Pass an existing plan
    # folder's name (e.g. SW-51860-SW-52867-<title>) to co-locate the dump with that ticket's work.
    ticket_folder = sys.argv[2] if len(sys.argv) > 2 else issue_id
    token = get_api_token()

    # Dump into the ticket's gitignored artifacts area, timestamped so re-reads never overwrite.
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    output_dir = (Path.home() / ".claude" / "tickets" / ticket_folder
                  / "artifacts" / "ticket-dumps" / timestamp)

...
```bash
pwsh -NoProfile -File "$HOME/.claude/skills/eli-prebind-validation/Run-PreBindValidation.ps1"
```

Pass `-NoBuild` to skip the build step (the slnx is already built):

```bash
pwsh -NoProfile -File "$HOME/.claude/skills/eli-prebind-validation/Run-PreBindValidation.ps1" -NoBuild
```

---
name: eli--run-ui-acceptance-tests-local
...
User invokes `/eli--run-ui-acceptance-tests-local` with one or more of:
...
```bash
pwsh -NoProfile -File "$HOME/.claude/scripts/Run-WebUiAcceptanceTest.ps1" -TicketFolder "<SW-XXXXX-title>" <FILTER_ARGS> [-NoBuild]
```
...
Both files land in the ticket's `artifacts/tests/` area (`~/.claude/tickets/<TicketFolder>/artifacts/tests/`) and survive after the script exits.

| What | Path | Notes |
|------|------|-------|
| Test output (text + TRX) | `~/.claude/tickets/<TicketFolder>/artifacts/tests/{branch}_{project}_{filters}_{timestamp}.txt` and `.trx` | **Same location and naming as any other test run via `Run-DotnetTest.ps1`.** This script delegates Step 5 to Run-DotnetTest.ps1, so the location is consistent across all tests. |
| Website stdout | `~/.claude/tickets/<TicketFolder>/artifacts/tests/Run-WebUiAcceptanceTest_{timestamp}_website.txt` | What `Swyfft.Web` printed during the test — written **every run** (pass or fail). Always read this when diagnosing a failure: backend exceptions (e.g., the SW-50063 KeyNotFoundException) only surface here. The script also tails the last 50 lines to stdout. |

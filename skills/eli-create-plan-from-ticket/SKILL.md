...
Check for an existing plan covering this ticket: scan the header block of each
`~/.claude/plans/*/plan.md` for the ticket ID (folders are named for the work title, not the
ticket, so match on the `SW-XXXXX` IDs recorded inside `plan.md`). If found, offer three options:
...
Path: `~/.claude/plans/{work-title-slug}/plan.md` — one folder per plan.

**Folder naming — derive from the ticket title(s); never invent a slug.**
- **Single ticket:** slugify the ticket's YouTrack title (lowercase, hyphenated); truncate to a
  reasonable length (~60 chars) if the title is long.
- **Multiple tickets:** a slug that captures all their titles in one concise descriptor. There is
  no "primary" ticket — don't pick one.
- Never a free-form or whimsical name, and never seeded from the conversation text. The folder name
  comes from the ticket title(s) only. (This is the fix for plan files landing under model-invented
  names.)

**Everything for the plan lives in this folder** — `plan.md` plus every artifact the plan produces
or references (rater dumps, diffs, logs, scratch data). Never scatter artifacts loose in `plans/`
or in sibling `*-dumps/` / `*-logs/` folders.

**Record the ticket IDs inside `plan.md`** — since the folder name no longer carries them, open the
plan with a header block listing every associated `SW-XXXXX` and its YouTrack link.
...
> **Plan written to `~/.claude/plans/{work-title-slug}/plan.md`. Execute now? (y/n)**

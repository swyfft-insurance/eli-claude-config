...
If seeding is needed, use the `/eli--seed` skill first.

## Step 2: Run the script

The script builds, then runs all 3 projects concurrently with output saved to the ticket's
`artifacts/tests/` area (`~/.claude/tickets/<TicketFolder>/artifacts/tests/`).

```bash
pwsh -NoProfile -File "$HOME/.claude/skills/eli--prebind-validation/Run-PreBindValidation.ps1" -TicketFolder "<SW-XXXXX-title>"
```

Pass `-NoBuild` to skip the build step (the slnx is already built):

```bash
pwsh -NoProfile -File "$HOME/.claude/skills/eli--prebind-validation/Run-PreBindValidation.ps1" -TicketFolder "<SW-XXXXX-title>" -NoBuild
```

...
5. **Verify.** Run the ByPeril Excel validation tests again (baselines auto-update on the run; review the diff) — now it also reflects the version wiring — plus `/prebind-captured-asserts`; confirm the final diff matches the scope with nothing unexpected, and the C# premium == Excel across all indices.

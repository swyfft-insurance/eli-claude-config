...
| Gated writes get a plain yes/no | "1a. Yes 1b. No" before a `gh pr create`, YouTrack update, Slack post, commit or push | "Run it? (y/n)" | The pretooluse hook accepts only a bare "y"/"yes" for those writes, so lettered options hand Eli an answer the hook rejects |
...
The same holds for the rules and the repo docs already loaded this session. A question one of them
answers is not a question. Re-read the file before asking.

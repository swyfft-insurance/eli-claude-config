...
- **Quote only what needs quoting.** An inline reply posts directly under the comment it answers,
  so the reviewer already has the text and a full re-quote is noise. In an inline reply, quote only
  when you're answering one sub-part of a multi-point comment, and quote just that sub-part. A
  top-level comment is anchored to nothing, so there quote each point and reply under it. Never use
  `#1`, `#2` etc. as labels — GitHub renders those as issue/PR links.
- **Always paste the reviewer's verbatim text — never a placeholder.** When drafting replies for Eli's approval, every draft must include the actual reviewer comment in the blockquote, copied word-for-word. NEVER substitute a placeholder like `(quote Justin)` / `(reviewer's comment here)`. Eli approves the reply against what it answers; a placeholder forces him to cross-reference and makes the draft impossible to judge. Show the real quote and the real reply, together, every time. This governs the draft you show Eli, never the posted body. The bullet above governs what the posted body quotes.
